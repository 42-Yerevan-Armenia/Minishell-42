...
/*   Updated: 2022/11/28 19:26:49 by vaghazar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	free_spl_pipe(t_list_spl_pipe **list)
{
	while ((*list)->head)
	{
		...
		if (((*list)->head)->fd_in != 0 && close((*list)->head->fd_in) == -1)
			ft_perror("minisehll: close: ");
		if (((*list)->head)->fd_out != 1 && close((*list)->head->fd_out) == -1)
			ft_perror("minisehll: close: ");
